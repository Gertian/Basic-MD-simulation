...
class Object{
	...
	public:
		...
		Object getFriendAt(unsigned int i)const{return *(_friends.at(i));}
		vector<Object> getFriends(){
			vector<Object> output;
			for(unsigned int i = 0; i<_friends.size(); i++){
				output.push_back(*(_friends.at(i)));
			}
			return output;
		} 
...
class Simulation{
	...
	public:
		...
		Object getObjectAt(unsigned int i)const{return(_objects.at(i));}
	public:
		//************************
		...
		vector<Vec> getPositions(){
			...
			for(unsigned int i = 0; i<_amountobjects; i++){
				output.push_back(_objects.at(i).getPosition());
			}
			return output;
		}
		...
		vector<Vec> getSpeeds(){
			vector<Vec> output;
			for(unsigned int i = 0; i<_amountobjects; i++){
				output.push_back(_objects.at(i).getSpeed());
			}
			return output;
		}
	...
	public:	
		...
		void shiftPositions(vector<Vec> a){
			for(unsigned int i = 0; i<_amountobjects; i++){
				_objects.at(i).shiftPosition(a.at(i),_systemsize);
			}
		}
		void shiftSpeeds(vector<Vec> a){
			for(unsigned int i = 0; i<_amountobjects; i++){
				_objects.at(i).shiftSpeed(a.at(i));
			}
		}
		void multiplySpeeds(double a){
			for(unsigned int i = 0; i<_amountobjects; i++){
				_objects.at(i).multiplySpeed(a);
			}
		}
	...
	public:
		...
		//x -> x - nint(x/_systemsize)*_systemsize. 
		//the last equation helps us to keep the perioded boundary conditons in the simulation.
		//*****************************************************************************************************
		void updateFriends(){
			for(unsigned int i = 0; i < _amountobjects; i++){
				_objects.at(i).clearFriends();
				for(unsigned int j = 0; j < _amountobjects; j++){
					if(i != j){	
						double xtemp = calcDistance(_objects.at(i).getPosition().getX(),_objects.at(j).getPosition().getX(),_systemsize);
						double ytemp = calcDistance(_objects.at(i).getPosition().getY(),_objects.at(j).getPosition().getY(),_systemsize);
						double ztemp = calcDistance(_objects.at(i).getPosition().getZ(),_objects.at(j).getPosition().getZ(),_systemsize);
						...
						if(sqrt(xtemp*xtemp + ytemp*ytemp + ztemp*ztemp) < _rcutof){
							_objects.at(i).addFriend(&_objects.at(j));
						}
					}
	...
	public:
		...
		double calcAutoCorrelation(){
			...
			for(unsigned int i = 0; i<_amountobjects; i++){
				currentguess += _initialspeeds.at(i)*currentspeeds.at(i);
			}
			...
			for(unsigned int i = 0; i<_amountobjects; i++){
				for(unsigned int j = 0; j<i; j++){
					if(i!=j){
						...
						output.at(floor(distance/_subslength)) = output.at(floor(distance/_subslength))+1;
					}
				}
			}
			return output;
	...
	public:
		...
		double calcKineticEnergy(){
			double output = 0;
			for(unsigned int i = 0; i < _amountobjects; i++){
				output += 0.5*pow(_objects.at(i).getSpeed().norm(),2);
			}
			return output;
		}
	public:
...
void simulate(Simulation& sim,double inittime,double time, double twriter, unsigned int scalesteps, double timestep, double temp){
	...
	for(unsigned int i = 0; i<sim.getSubs(); i++){
		...
		corr << (i+1.)*sim.getSubslength() << "\t" <<summedbinned.at(i)*prefactor << endl; 
	}
